...
def _toponym_ref(modern_name: str, region: str | None) -> str:
    """Cross-file toponym ref. Same shape as the rest of the project."""
...
def build_events(
    ...
    *,
    ...
    """Pure function: CSV rows → JSONL events. Source row first
    (optional); then alternating toponym + attestation. Toponym refs
    dedupe (kernel rejects duplicate adds); attestations don't
    dedupe so each Roll entry is independently auditable."""
...
def ingest(csv_path: str | Path, out_path: str | Path, *, apply: bool = False) -> dict[str, int]:
    """Read CSV, write JSONL events to ``out_path``. Counts:
    csv_rows_scanned, toponym_events, attestation_events, written."""
