...
- **`etymon_variant`** (wiktextract forms-mining): Phase 3a task once
  the forms-mining ingester is re-cut to emit JSONL events. **Exception
  (wyrd-eni4.2.2):** the variant rail's `toponymic-surface`-tagged
  alt-spellings ARE L2 — paid grounded authoring, not re-derivable — and
  round-trip through `data/mining/_variants.jsonl` (`_type: variant` rows
  `{ref, form, tags}`), replayed into `etymon_variant` by
  `build._insert_variant_rows`. The ~5.8M wiktextract forms remain L1.
