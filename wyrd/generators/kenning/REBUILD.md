...
| element-gloss backfill (`reflex_etymon` links for unglossed generation surfaces, wyrd-u9k6) | ✅ yes — `data/mining/_element_glosses.jsonl` (deterministic consensus) + `_element_gloss_adjudications.jsonl` (LLM picks), replayed by `run_full_enrichment`'s element-gloss pass (`apply_element_glosses`). **Re-mining is safe**: `mine-element-glosses` MERGES into the ledger by `(surface, position)` (existing wins), so a fresh — necessarily partial — census never shrinks the accumulated corpus (wyrd-5f5w). `--overwrite` is a destructive from-scratch rebuild (drops every surface the new census doesn't re-derive) — don't use it on the committed ledger. | — |
