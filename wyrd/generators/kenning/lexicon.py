...
def seed_meaning_synsets(db: LexiconDB) -> dict[str, int]:
    ...
    for entry in entries:
        ...
        if row["notes"] != notes:
            ...
            unchanged += 1
    # Pass 2: hypernym links. Resolve each label's hypernym to an id
    # via label_to_id; missing hypernyms (typo or out-of-catalog) are
    # logged but don't fail the seed — the row stays with hypernym=NULL.
    for entry in entries:
        hypernym_label = entry.get("hypernym")
        if hypernym_label is None:
            continue
        if hypernym_label not in label_to_id:
            # Caller's seed file references an unknown hypernym; skip
            # the link rather than fail the whole seed run.
            continue
        db.conn.execute(
...
def get_meaning_preserving_candidates(
    ...
    *,
    ...
    A single (target, candidate) pair can produce multiple rows if
    they share multiple synsets — caller decides whether to dedupe.
