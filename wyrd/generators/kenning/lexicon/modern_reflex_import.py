...
def import_modern_reflexes(db, jsonl_path: str | Path, *, apply: bool = True) -> dict:
    ...
    for lineno, line in enumerate(path.read_text(encoding="utf-8-sig").splitlines(), 1):
        ...
        if row.get("_type") != "modern_reflex":
            continue
        _process_reflex_row(db, conn, row, apply=apply, counts=counts)
    if apply:
        db.commit()
    return dict(counts)


def _process_reflex_row(db, conn, row: dict, *, apply: bool, counts: Counter) -> None:
    ...
    for form in forms:
        _apply_reflex_form(db, conn, form, m_id, m_cog, row, apply=apply, counts=counts)


def _apply_reflex_form(
    db, conn, form: str, m_id: int, m_cog, row: dict, *, apply: bool, counts: Counter
) -> None:
