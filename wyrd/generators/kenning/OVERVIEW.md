...
- **`COVERAGE.md`** — rolling log of place-name decomposition rate
  ...
  not the north star.
- **`REBUILD.md`** — the full-rebuild runbook: wipe `~/.wyrd/lexicon.db`
  and replay it from L2 JSONL, then re-export the L4 bundle. Read it
  before any `rebuild-from-jsonl --with-enrichment` run — it records
  the L3-only layers a full wipe silently drops (empirical, attestations,
  forms-variants, country) and the order to restore them. Kept complete
  by `data/mining/_rebuild_layers.json` + `tests/test_kenning_rebuild_runbook.py`
  and the **`rebuild-runbook-currency-reviewer`** (in `AGENT-REVIEWERS.md`,
  spec under `.reviewers/`), which fail/flag a new data-population CLI that
  isn't documented as a rebuild step.
- **`L2_L3_BOUNDARY.md`** — column-by-column map of which data is L2
  (replayed from JSONL) vs L3 (regenerated by enrichment), plus the
  bulk-sources / S3 workflow. The reference the rebuild runbook leans on.
