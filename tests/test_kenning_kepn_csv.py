...
def _briggs_fixture(path: Path) -> Path:
    ...
    rows = [
        ...
        {
            "_type": "etymon",
            "ref": "old-english:Earda",
            "canonical_form": "Earda",
            "language": "old-english",
            "tags": ["male name"],
            "glosses": ["a personal name"],
        },
        {"_type": "citation", "etymon_ref": "old-english:Earda"},
        {
            "_type": "etymon",
            "ref": "old-english:Æþelstan",
            "canonical_form": "Æþelstan",
            "language": "old-english",
            "tags": ["male name"],
            "glosses": ["a personal name"],
        },
...
def test_external_citation_only_when_referenced(tmp_path):
    csv = _write_csv(
        ...
        [
            (
                "Reffed",
                "'Ash spring'.",
                "æsc Old English - Ash; wella Old English - Spring.",
                "E. Ekwall; DEPN",
            ),
    ...
    def srcs(form):
        r = c.execute(
            ...
            "JOIN etymon_citation ci ON ci.etymon_id=e.id WHERE e.canonical_form=?",
            (form,),
        ).fetchone()
    ...
    csv = _write_csv(
        ...
        [
            (
                "Grimston",
                "'Grimr's farmstead'.",
                "tūn Old English - A farmstead; Personal name (Old Norse) Old Norse - Personal name",
                "DEPN",
            ),
        ],
    )
...
def test_whitelist_quarantines_unknown_form(tmp_path):
    csv = _write_csv(
        ...
        [
            (
                "Junkton",
                "'Junk'.",
                "tūn Old English - A farmstead; xyzzy Old English - Nonsense.",
                "",
            ),
        ],
    )
    ...
    stats = emit_kepn_jsonl(
        csv, out, county="Shire", name_index={}, element_whitelist=frozenset({"tūn"})
    )
